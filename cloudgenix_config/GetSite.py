...
SCRIPT_NAME = 'CloudGenix: Example script: Get Site'
SCRIPT_VERSION = "v1"
...
def get():
    ...
    if CLOUDGENIX_AUTH_TOKEN:
        ...
        if cgx_session.tenant_id is None:
            print("Prisma SD-WAN login failure, please check auth token.")
            return
    elif user_email:
        cgx_session.interactive.login(user_email, user_password)
        if cgx_session.tenant_id is None:
            print("Prisma SD-WAN login failure, please check username/password.")
            return            
    else:
        print("No Primsa SD-WAN login infromation found in cloudgenix_settings.py")
        return
...
def tenant():
    ...
    # create file-system friendly tenant str.
    tenant_str = "".join(x for x in cgx_session.tenant_name if x.isalnum()).lower()
    print("Connected to tenant: " + tenant_str)
    return
    # end of script, run logout to clear session.
